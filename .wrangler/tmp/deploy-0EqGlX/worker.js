...
var ClinicScreenContainer = class extends Container {
  constructor(ctx, env) {
    super(ctx, env);
    this.defaultPort = 8080;
    this.sleepAfter = "1h";
    // stay warm across a session; scale to zero after idle
    this.envVars = {};
    for (const k of PASS_THROUGH) {
      const v = env[k];
      if (v) this.envVars[k] = v;
    }
  }
  static {
    __name(this, "ClinicScreenContainer");
  }
};
var worker_default = {
  ...
  async scheduled(_event, env, ctx) {
    ctx.waitUntil(
      getContainer(env.CONTAINER).fetch(new Request("https://container.internal/login")).then(() => void 0).catch(() => void 0)
    );
  }
};
export {
  ClinicScreenContainer,
