...
function TriggerButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="text-xs font-medium text-amber-700 transition hover:text-amber-900 disabled:opacity-50"
    >
      ...
      {state.tempPassword && (
        <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
          ...
          <div className="mt-2 flex items-center gap-2">
            <code className="flex-1 select-all rounded border border-amber-200 bg-white px-2.5 py-1.5 font-mono text-sm tracking-wide text-slate-800">
              {state.tempPassword}
            </code>
            <button
              type="button"
