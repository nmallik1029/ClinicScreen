...
export function Card({
  ...
  return (
    <div
      {...rest}
      className={`rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition-colors dark:border-slate-800 dark:bg-slate-900 ${className}`}
    >
      {children}
    </div>
  );
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full rounded border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-blue-400 ${props.className ?? ""}`}
    />
  );
}
...
export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={`w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:focus:border-blue-400 ${props.className ?? ""}`}
    />
  );
}
...
export function Button({
  ...
  const styles = {
    ...
    danger:
      "border border-red-200 bg-white text-red-600 hover:bg-red-50 dark:border-red-900/60 dark:bg-slate-900 dark:text-red-400 dark:hover:bg-red-950/50",
  }[variant];
  return (
    <button
      {...props}
...
export function Tabs({
  ...
  return (
    <nav className="mb-6 grid gap-2 rounded-lg border border-slate-200 bg-white p-2 shadow-sm transition-colors dark:border-slate-800 dark:bg-slate-900 sm:grid-cols-2 lg:grid-cols-5">
      {tabs.map((t) => {
        ...
        return (
          <Link
            ...
            className={`group relative overflow-hidden rounded-md border px-3 py-3 text-left transition duration-200 ${
              isActive
                ? "border-blue-200 bg-blue-50 text-blue-950 shadow-sm dark:border-blue-900/70 dark:bg-blue-950/50 dark:text-blue-100"
                : "border-transparent text-slate-600 hover:border-slate-200 hover:bg-slate-50 hover:text-slate-950 dark:text-slate-400 dark:hover:border-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-100"
            }`}
          >
            <span
              ...
              {metric ? (
                <span
                  className={`shrink-0 rounded-md px-2 py-1 text-xs font-semibold ${
                    isActive
                      ? "bg-white text-blue-700 dark:bg-blue-900/60 dark:text-blue-200"
